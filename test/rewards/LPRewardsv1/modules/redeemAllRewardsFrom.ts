...
export default function run(): void {
	...
	it('should transfer all rewards for a token', async function () {
		const { contract, deployer, rewardsToken, uniswapPool } = fixture;
		await contract.redeemAllRewardsFrom(uniswapPool.address, true);

		expect(
			await rewardsToken.balanceOf(contract.address),
			'contract rewards balance mismatch after redemption',
		).to.eq(rewards.sub(deployerUniRewards));
	...
	it('should transfer all rewards for all tokens in ETH', async function () {
		const { contract, deployerSigner, rewardsToken, uniswapPool } = fixture;

		const balanceBefore = await deployerSigner.getBalance();

		const tx = await contract.redeemAllRewardsFrom(uniswapPool.address, false);
		...
		expect(
			...
			'deployer rewards balance mismatch after redemption',
		).to.eq(deployerUniRewards.sub(gasUsed));
	});
	...
	describe('should update totalRewardsRedeemed', async function () {
		it('correctly', async function () {
			...
			await contract.redeemAllRewardsFrom(uniswapPool.address, true);
			expect(await contract.totalRewardsRedeemed()).to.eq(deployerUniRewards);
		});
	...
	it('should zero rewardsBalanceOfFor token', async function () {
		const { contract, deployer, uniswapPool } = fixture;
	...
	it('should emit RewardPaid event', async function () {
		...
		await expect(contract.redeemAllRewardsFrom(uniswapPool.address, true))
